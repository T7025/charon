...
TEST(BarnesHutTreeUniverse, CompareToBruteForce) {
    auto settings = std::make_shared<Settings>();
    settings->fileSettings->enableFileOutput = false;
    settings->universeSettings->timeStep = 0.0001;
    settings->universeSettings->gravitationalConstant = 1.0;
    settings->universeSettings->useCollisions = false;
...
TEST(BarnesHutUniverse, CompareToBruteForce) {
    ...
    bruteForceUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    bruteForceUniverse->calculateFirstStep();
    barnesHutUniverse->calculateFirstStep();
    fp maxDeviation = 0;
    for (int i = 0; i < 10; ++i) {
        bruteForceUniverse->calculateNextStep();
        barnesHutUniverse->calculateNextStep();
        ...
        std::sort(positions1.begin(), positions1.end(), comp);
        std::sort(positions2.begin(), positions2.end(), comp);
...
TEST(BarnesHutUniverse, CompareToBarnesHutTree) {
    ...
    settings->universeSettings->useCollisions = false;
    settings->universeSettings->number_bodies = 256;
    settings->simulatorSettings->barnesHutCutoff = 0.5;
    auto console = std::make_shared<spdlog::logger>("console", std::make_shared<spdlog::sinks::null_sink_mt>());

    auto barnesHutTreeUniverse = std::make_shared<BarnesHutTreeUniverse>(settings, console);
    auto barnesHutUniverse = std::make_shared<BarnesHutUniverse>(settings, console);

    barnesHutTreeUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutTreeUniverse->calculateFirstStep();
    barnesHutUniverse->calculateFirstStep();
    fp maxDeviation = 0;
    for (int i = 0; i < 10; ++i) {
        barnesHutTreeUniverse->calculateNextStep();
        barnesHutUniverse->calculateNextStep();
...
TEST(BarnesHutUniverse, BarnesHutMultiThreadUniverse) {
    ...
    barnesHutUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutMultiThreadUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutUniverse->calculateFirstStep();
    barnesHutMultiThreadUniverse->calculateFirstStep();
    fp maxDeviation = 0;
    for (int i = 0; i < 100; ++i) {
        barnesHutUniverse->calculateNextStep();
        barnesHutMultiThreadUniverse->calculateNextStep();
...
TEST(BarnesHutUniverse, BarnesHutOffloadUniverse) {
    ...
    barnesHutUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutOffloadUniverse->initBodies(std::make_shared<RandomCubeUniverseShape>());
    barnesHutUniverse->calculateFirstStep();
    barnesHutOffloadUniverse->calculateFirstStep();
    fp maxDeviation = 0;
    for (int i = 0; i < 100; ++i) {
        barnesHutUniverse->calculateNextStep();
        barnesHutOffloadUniverse->calculateNextStep();
        auto positions1 = barnesHutUniverse->getPositions();
