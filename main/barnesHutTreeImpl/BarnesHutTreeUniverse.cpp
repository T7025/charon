...
#include "BarnesHutTreeUniverse.h"
#include <spdlog/fmt/ostr.h>
#include <universeShapes/UniverseShape.h>
#include <utility>

BarnesHutTreeUniverse::BarnesHutTreeUniverse(std::shared_ptr<Settings> settings, std::shared_ptr<spdlog::logger> console)
        : AbstractUniverse(std::move(settings), std::move(console)),
          errorRate{this->settings->simulatorSettings->barnesHutCutoff} {}

void BarnesHutTreeUniverse::addBody(fp &&mass, Vector3 &&position, Vector3 &&velocity, Vector3 &&acceleration) {
...
size_t BarnesHutTreeUniverse::bodyCount() const {
    return mass.size();
}

void BarnesHutTreeUniverse::fillTree() {
...
Vector3 BarnesHutTreeUniverse::calcAcceleration(const size_t target) const {
    return octTree->calculateAcceleration(position[target], errorRate);
}

void BarnesHutTreeUniverse::calculateFirstStep() {
    calcNextPosition();
    fillTree();
    for (size_t i = 0; i < bodyCount(); ++i) {
...
void BarnesHutTreeUniverse::calculateNextStep() {
    fillTree();
...
void BarnesHutTreeUniverse::logInternalState() const {
...
void BarnesHutTreeUniverse::calcNextPosition() {
    for (unsigned i = 0; i < bodyCount(); ++i) {
...
std::vector<Vector3> BarnesHutTreeUniverse::getPositions() const {
    return position;
}

fp BarnesHutTreeUniverse::calcEnergy() const {
    fp result = 0;
    for (unsigned i = 0; i < bodyCount(); ++i) {
...
Vector3 BarnesHutTreeUniverse::calcMomentum() const {
