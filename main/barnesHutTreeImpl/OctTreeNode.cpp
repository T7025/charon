...
void OctTreeNode::calculateAcceleration(Vector3 &result, const Vector3 &targetPosition, const fp &errorRate) const {
    // if node is self
    ...
    // if leaf node
        ...
        result += diff * (totalMass / boost::math::pow<3>(norm));
        return;
    } else {
        for (const auto &child : children) {
            if (child.totalMass > 0) {
                child.calculateAcceleration(result, targetPosition, errorRate);
            }
        }
    }
}
...
Vector3 OctTree::calculateAcceleration(const Vector3 &targetPosition, const fp &errorRate) const {
    Vector3 result{0,0,0};
    root.calculateAcceleration(result, targetPosition, errorRate);
    return result;
}
