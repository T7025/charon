...
void BruteForceUniverse::addBody(fp mass, Vector3 position, Vector3 velocity, Vector3 acceleration) {
...
size_t BruteForceUniverse::bodyCount() const {
    return mass.size();
}
...
void BruteForceUniverse::logInternalState() const {
...
std::vector<Vector3> BruteForceUniverse::getPositions() const {
    return position;
}

fp BruteForceUniverse::calcEnergy() const {
    fp result = 0;
    for (unsigned i = 0; i < bodyCount(); ++i) {
...
Vector3 BruteForceUniverse::calcMomentum() const {
    const auto size = bodyCount();
